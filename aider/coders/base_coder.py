    ...
    def send_new_user_message(self, inp):
        ...
        if(self.perform_refinement):
            ...
            if improved_message.strip():
                wrapped_message = ("Please review following autogenerated summary: "
                                   f"{improved_message} and perform task specified by the user, "
                                   "if summary contradicts user's request, follow user's request, not summary")
                self.cur_messages += [
                    dict(role="user", content=wrapped_message),
                ]
